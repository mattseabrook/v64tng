...
void renderFrameD2D() {
    ...
    // Convert RGB to BGRA using persistent buffer
    for (size_t i = 0, j = 0; i < pixelData.size() && j < bgraBuffer.size(); i += 3, j += 4) {
        bgraBuffer[j] = pixelData[i + 2];     // Blue
        bgraBuffer[j + 1] = pixelData[i + 1]; // Green
        bgraBuffer[j + 2] = pixelData[i];     // Red
        bgraBuffer[j + 3] = 255;              // Alpha
    }

    // Update bitmap
    HRESULT hr = bitmap->CopyFromMemory(nullptr, bgraBuffer.data(), MIN_CLIENT_WIDTH * 4);
    if (FAILED(hr)) {
    ...
    // Draw scaled bitmap using global scaleFactor
    D2D1_RECT_F destRect = D2D1::RectF(
        0.0f,
        0.0f,
        MIN_CLIENT_WIDTH * scaleFactor,
        MIN_CLIENT_HEIGHT * scaleFactor
    );
