...
struct MegatextureState
{
    ...
    std::vector<int> edgeLookup;   // (mapY * mapWidth + mapX) * 4 + side -> edge index, -1 if none
    ...
    uint64_t version;                              // Bumped whenever MTX pixel data changes
};
...
// Install one RGBA image as a continuous wall texture across every map cell
// matching wallTile on the requested face. Resampling is completed once here;
// renderers only ever sample the permanent processed buffer.
bool loadRaycastWallTexture(const TileMap& map,
                            const uint8_t* rgba,
                            int sourceWidth,
                            int sourceHeight,
                            int targetWidth,
