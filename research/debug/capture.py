...
MODE_GROUPS = {
    "core": ["grv", "resource", "audio"],
    "media": ["vdx", "audio"],
    "full": ["grv", "resource", "vdx", "audio", "input"],
}
...
class TraceWriter:
    def __init__(self, root: Path, source: str):
        ...
        self.auto_mark_lock = threading.Lock()
        self.auto_mark_count = 0
        self.last_auto_mark_monotonic = -1.0
        self.closed = False
    ...
    def auto_mark(self) -> None:
        """Emit one numbered mark for duplicate reports of the same F9 press."""
        now = time.monotonic()
        with self.auto_mark_lock:
            if now - self.last_auto_mark_monotonic < 0.25:
                return
            self.last_auto_mark_monotonic = now
            self.auto_mark_count += 1
            label = f"mark-{self.auto_mark_count}"
        self.mark(label)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=64,
        metavar="BYTES",
        help="bounded script bytes captured at each GRV dispatch (64-2048; default 64)",
    )
        ...
        def on_message(message: dict[str, Any], data: bytes | None) -> None:
            if message.get("type") == "send":
                ...
                if (
                    ...
                    and payload.get("message") == "0x0100"
                    and payload.get("wparam") == "0x78"
                ):
                    # Trace 20260809-170739 proved a four-millisecond F9 press
                    # can fall entirely between the 50 ms host polls. The
                    # hooked WM_KEYDOWN is lossless; auto_mark deduplicates it
                    # if GetAsyncKeyState observed the same physical press.
                    writer.auto_mark()
                if probe == "tracer.ready":
                    ...
                    if payload.get("audio_api_failures"):
                        print(
