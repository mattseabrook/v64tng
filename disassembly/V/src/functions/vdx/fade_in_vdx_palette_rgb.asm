...
; Verified role (static + cross-version): channel-staggered palette fade-in.
; Called after a still when CF4Ch bit 0 (GRV opcode 03h) is set; the caller
; clears the one-shot flag. Win32 counterpart: 0040AA45h.
; BP is zero; BP-relative globals use SS. DS is loaded from SS:D552h.
; Source RGB palette CF8Ch -> staging CB20h; 256 entries, floor(c*gain/256).
; Gains at CF4Eh/CF50h/CF52h start at 16 and saturate at 256, advancing
; by 8/4/2 respectively. Each upload waits for a fresh VGA retrace.
; There are 121 uploads: n=0..120, gain=min(256,16+n*(8,4,2)).
; Red reaches full at n=30, green at 60, blue at 120. This is not a
; uniform brightness fade, and its first upload is 1/16 intensity.
; Generated losslessly; edit names/comments only after preserving build identity.

%macro emit_fade_in_vdx_palette_rgb_part_00 0
    %%fragment_start:
fade_in_vdx_palette_rgb:
    %%insn_00f33:
    pusha ; 00F33 60
    %if ($ - %%insn_00f33) > 1
        %error "LONG_00F33"
