...
; Verified role (static + cross-version): GRV opcode 04h palette fade-out.
; Caller: run_grv_vm at 03B70h. Win32 counterpart: 0040A84Bh.
; Source RGB palette CF8Ch -> staging CB20h; floor(c*gain/256).
; Gains start at 256; red decreases by 2, green/blue by 4, clamped at zero.
; There are 129 retrace-synchronized uploads: n=0..128, gains
; (max(0,256-2*n), max(0,256-4*n), max(0,256-4*n)).
; Green/blue vanish halfway through, leaving a red tail. The final upload
; is black, then 0285Dh clears display memory. Source palette is retained.
...
fade_out_vdx_palette_rgb_and_clear:
    ...
    %%insn_01054:
    call clear_indexed_display_memory ; 01054 E80618
    %if ($ - %%insn_01054) > 3
        %error "LONG_01054"
    %endif
