...
; Corrected role: GRV22h snapshots display into XMS-backed saved background.
; DS comes from SS:D558h (display); ES from SS:D554h (conventional scratch).
; Unbanked path CF22h !=0 copies64000 bytes with REP MOVSW, then requests
; a65536-byte XMS move: source handle0, source offset D554h:0000,
; destination handle SS:D550h, destination offset0. Machine-code verified
; through capture of the XMS descriptor; actual XMS driver is not emulated.
; Banked path starts at row80, copies320 rows at stride D66Ah, and changes
; each FFh pixel to FEh while snapshotting. It moves each scratch bank into
; XMS at bank_index<<16. Static evidence for this banked path, not a capture.
; Win32 counterpart0040A786h also snapshots display to saved background.
; Earlier background-to-foreground/restore name had the direction reversed.
; Generated losslessly; edit names/comments only after preserving build identity.

%macro emit_snapshot_display_to_xms_background_part_00 0
    %%fragment_start:
snapshot_display_to_xms_background:
    %%insn_02591:
    push ds ; 02591 1E
    %if ($ - %%insn_02591) > 1
        %error "LONG_02591"
