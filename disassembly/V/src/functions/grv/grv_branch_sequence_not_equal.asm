...
; Opcode 1Ah caller 03BD0h. Compare successive variables against decoded elements, incrementing DL
; on mismatch. After terminal DH bit 7, jump via [DS:SI] if DL != 0; otherwise skip the two-byte
; target. DL is an eight-bit mismatch counter, not a boolean.
; Generated losslessly; edit names/comments only after preserving build identity.

%macro emit_grv_branch_sequence_not_equal_part_00 0
    %%fragment_start:
grv_branch_sequence_not_equal:
    %%insn_041f8:
    call grv_read_variable_operand ; 041F8 E80FFE
    %if ($ - %%insn_041f8) > 3
        %error "LONG_041F8"
    %endif
    ...
    %%insn_04202:
    call grv_decode_sequence_element ; 04202 E811FE
    %if ($ - %%insn_04202) > 3
        %error "LONG_04202"
    %endif
