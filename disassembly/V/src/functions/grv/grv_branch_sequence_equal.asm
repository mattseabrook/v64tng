...
; Opcode 23h caller 03C42h. DL starts at one and is cleared on any unequal element. After terminal
; DH bit 7, branch to the word target only if all elements matched; otherwise skip it.
; Generated losslessly; edit names/comments only after preserving build identity.

%macro emit_grv_branch_sequence_equal_part_00 0
    %%fragment_start:
grv_branch_sequence_equal:
    %%insn_042cb:
    call grv_read_variable_operand ; 042CB E83CFD
    %if ($ - %%insn_042cb) > 3
        %error "LONG_042CB"
    %endif
    ...
    %%insn_042d5:
    call grv_decode_sequence_element ; 042D5 E83EFD
    %if ($ - %%insn_042d5) > 3
        %error "LONG_042D5"
    %endif
