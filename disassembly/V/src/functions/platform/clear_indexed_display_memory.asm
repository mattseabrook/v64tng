...
; Static + fade-out caller 01054h. ES comes from SS:D558h. CF22h != 0 clears 7D00h words (64000
; bytes). Otherwise select banks through SS:D660h and clear 8000h words in each of five banks.
; Sets CF8Ah bank state on the banked path; preserves ES. Do not confuse this hardware display
; clear with the saved VDX background.
; Generated losslessly; edit names/comments only after preserving build identity.

%macro emit_clear_indexed_display_memory_part_00 0
    %%fragment_start:
clear_indexed_display_memory:
    %%insn_0285d:
    push es ; 0285D 06
    %if ($ - %%insn_0285d) > 1
        %error "LONG_0285D"
