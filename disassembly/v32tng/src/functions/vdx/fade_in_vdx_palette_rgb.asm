...
; Verified role (static + DOS correspondence): fade in the next VDX still.
; Caller 0040B16Dh tests/clears VIDEOFLAG 0200h (GRV opcode 03h).
; DOS counterpart: 00F33h. RGB source: 256 four-byte slots at 00420A40h;
; destination: packed RGB palette through [00421344h].
; Reset tick counter 0041F524h, then floor(c*gain/256), with gains
; min(256,tick*64), min(256,tick*32), min(256,tick*16).
; Channels reach full at ticks 4/8/16. Counter is advanced by the
; 66 ms multimedia timer installed at 00407485h: nominal duration 1056 ms.
; Counter loads are separate; a tick may change between channel reads.
; Unlike DOS, this is timer-driven and starts with zero gain if tick is zero.
