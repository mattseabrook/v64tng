...
; Verified role (static + DOS correspondence): GRV opcode 04h fade-out.
; Caller: run_grv_vm at 004023EAh. DOS counterpart: 00FC6h.
; Snapshot packed RGB palette [00421344h] into 256 four-byte slots at
; 00420A40h, reset 0041F524h, then scale from that fixed snapshot.
; Gains: max(0,256-tick*16), max(0,256-tick*32), max(0,256-tick*32).
; floor(c*gain/256); green/blue reach zero at tick 8, red at tick 16.
; Timer installed at 00407485h requests 66 ms: nominal duration 1056 ms.
; Counter loads are separate; a tick may change between channel reads.
; Finally zero 04B000h bytes (640*480 indexed pixels) at [0042133Ch]
; and request the full 640x480 presentation region. Retains RGB snapshot.
; Generated losslessly; preserve byte identity after edits.

%macro emit_fade_out_vdx_palette_rgb_and_clear_part_00 0
    %%fragment_start:
fade_out_vdx_palette_rgb_and_clear:
    %%insn_0040a84b:
    ...
    %%insn_0040aa06:
    call fill_memory_bytes ; 0040AA06 E8652A0000
    %if ($ - %%insn_0040aa06) > 5
        %error "LONG_0040AA06"
    %endif
