...
; Static + normal-path machine-code tests. Args (mmio handle, cached handle low word). Resource
; size 0042238Ch=0 defaults to 20000h. Direct path reads min(size,4000h) bytes into [0041F5F4h];
; cached path copies min(size,FFF0h) from cached block+10h. Sets read/write/available counters;
; when refills disabled, pads through 20000h with FFh. Cached continuation is sought immediately
; unless 00421422h bit1 requests deferral, in which case handle is parked at 0041F5ECh. Ignores
; mmioRead return count. SEH normal paths tested; exception handling not emulated.
; Generated losslessly; preserve byte identity after edits.

%macro emit_initialize_vdx_buffered_reader_part_00 0
    %%fragment_start:
initialize_vdx_buffered_reader:
    %%insn_00409b1d:
    ...
    %%insn_00409bda:
    call get_cached_resource_block_and_increment_uses ; 00409BDA E8C0070000
    %if ($ - %%insn_00409bda) > 5
        %error "LONG_00409BDA"
    %endif
    ...
    %%insn_00409bf7:
    call copy_memory_overlap_safe ; 00409BF7 E8142E0000
    %if ($ - %%insn_00409bf7) > 5
        %error "LONG_00409BF7"
    %endif
    ...
    %%insn_00409c28:
    call seek_to_cached_resource_offset ; 00409C28 E8C1070000
    %if ($ - %%insn_00409c28) > 5
        %error "LONG_00409C28"
    %endif
...
%macro emit_initialize_vdx_buffered_reader_part_01 0
    %%fragment_start:
    %%insn_00409c98:
    ...
    %%insn_00409cb9:
    call fill_memory_bytes ; 00409CB9 E8B2370000
    %if ($ - %%insn_00409cb9) > 5
        %error "LONG_00409CB9"
    %endif
