...
; Static + machine-code checks. Public handle is one-based; 20-byte slot at
; 0041F5F8h+(handle-1)*20 stores pointer,+4 use count,+8 resource ID,+12 continuation offset,+16
; byte size. Increment use count (32-bit wraparound), return pointer. Counter increments
; establish frequency semantics, not an LRU timestamp. Original guard only rejects handle >
; slot_count+1; no lower-bound validation.
; Generated losslessly; preserve byte identity after edits.

%macro emit_get_cached_resource_block_and_increment_uses_part_00 0
    %%fragment_start:
get_cached_resource_block_and_increment_uses:
    %%insn_0040a39f:
