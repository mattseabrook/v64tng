...
; Static + controlled-free machine-code cases. Repeatedly scan slots below count 00420A04h for
; nonzero byte size and smallest unsigned use count below 40000000h; ties keep the first slot.
; Original test at 0040A30Eh requires selected index > 0: selecting slot ZERO fails without
; eviction. For accepted slots, clear resource flag, usage/size, free pointer, zero pointer;
; accumulate freed sizes until requested total met. Returns 1 success, -1 failure. Failure can
; follow partial eviction. Even requested=0 evicts a block first. Do not silently repair slot-
; zero behavior or relabel this as LRU.
; Generated losslessly; preserve byte identity after edits.

%macro emit_evict_least_used_resource_blocks_part_00 0
    %%fragment_start:
evict_least_used_resource_blocks:
    %%insn_0040a29f:
    ...
    %%insn_0040a325:
    call clear_resource_cache_flag ; 0040A325 E8E0FBFFFF
    %if ($ - %%insn_0040a325) > 5
        %error "LONG_0040A325"
    %endif
