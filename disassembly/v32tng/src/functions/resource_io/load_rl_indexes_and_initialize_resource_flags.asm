...
; Caller initialize_grv_runtime_and_load_script at 004012EAh. Formats %s.rl from the 21 archive
; names at 0041B038h; mmioOpenA/mmioRead/mmioClose IAT 00424598h/594h/590h. Uses a 4000h-byte
; temporary read buffer, allocates and copies each actual read result into (size,pointer) pairs
; at 004223A0h. Adds read_size>>3 to a local count, but overwrites that count with 10000h before
; allocating/zeroing resource flags at [00422448h]. Seeds the PRNG from timeGetTime. The read is
; capped at 16 KiB, not a whole-file read loop.
; Generated losslessly; preserve byte identity after edits.

%macro emit_load_rl_indexes_and_initialize_resource_flags_part_00 0
    %%fragment_start:
load_rl_indexes_and_initialize_resource_flags:
    %%insn_00401000:
    ...
    %%insn_004010d5:
    call copy_memory_overlap_safe ; 004010D5 E836B90000
    %if ($ - %%insn_004010d5) > 5
        %error "LONG_004010D5"
    %endif
    ...
    %%insn_00401116:
    call seed_grv_random_state ; 00401116 E895590000
    %if ($ - %%insn_00401116) > 5
        %error "LONG_00401116"
    %endif
