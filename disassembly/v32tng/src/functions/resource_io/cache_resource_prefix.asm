...
; Static + normal-path allocation/read tests. Args (mmio handle,resource ID,resource offset),
; size in0042238Ch. Cache min(size,10000h) bytes in first empty/new slot; at256 slots or
; allocation failure, evict smallest use-count below40000000h and retry. Unlike
; evict_least_used_resource_blocks, slot zero is accepted. Store resource ID and continuation (0
; for size<10000h; offset+10000h otherwise), increment uses, return slot+1; return-1 if no
; candidate/allocation recovery. Caller00401E9Dh stores the handle as ONE BYTE, so handle256
; becomes zero. Read return count ignored. SEH exception paths not emulated.
; Generated losslessly; preserve byte identity after edits.

%macro emit_cache_resource_prefix_part_00 0
    %%fragment_start:
cache_resource_prefix:
    %%insn_00409f1a:
    ...
    %%insn_00409ff8:
    call clear_resource_cache_flag ; 00409FF8 E80DFFFFFF
    %if ($ - %%insn_00409ff8) > 5
        %error "LONG_00409FF8"
    %endif
    ...
    %%insn_0040a144:
    call clear_resource_cache_flag ; 0040A144 E8C1FDFFFF
    %if ($ - %%insn_0040a144) > 5
        %error "LONG_0040A144"
    %endif
...
%macro emit_cache_resource_prefix_part_01 0
    %%fragment_start:
    %%insn_0040a24f:
