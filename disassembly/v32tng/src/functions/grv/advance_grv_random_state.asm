...
; Static + machine-code checks of six seeds and 64 successive steps each. If all three state
; words are zero, substitute E6h,1Ch,0Ch. Performs 16 feedback rounds using shifts/XOR and
; explicit carry-like temporaries; stores 16-bit words without byte truncation. Returns
; (state0<<8)|state1 in EAX, not just a byte. Range wrapper 00401572h uses AX, normally its low
; byte. Superficially resembles DOS 042F1h, but identical output streams are not assumed.
; Executable reference arithmetic: grooviev1/verify_win32_grv_helpers.py.
; Generated losslessly; preserve byte identity after edits.

%macro emit_advance_grv_random_state_part_00 0
    %%fragment_start:
advance_grv_random_state:
    %%insn_00406b14:
